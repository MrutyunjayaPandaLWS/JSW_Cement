...
class MSP_MyRedemptionDetails: BaseViewController, popUpDelegate {
    ...
    @IBOutlet var referNubmer: UILabel!
    ...
    var descData = ""
        ...
        override func viewDidLoad() {
            ...
            referNubmer.text = prodRefNo
            
            var totalImgURL = "\(productCatalogueImgURL)"+"\(productImage)"
            productImageView.sd_setImage(with: URL(string: totalImgURL), placeholderImage: UIImage(named: "ic_default_img"))
         
            
            categoryTypeLabel.text = "Catagory / \(catagryName)"
            productNameLabel.text = productName
            descriptions.text = descData
            termsandConditions.text = termsandContions
...
//                //                print(calcValues)
//                if Int(pointBalance) ?? 0 <= Int(productPoints) ?? 0 {
